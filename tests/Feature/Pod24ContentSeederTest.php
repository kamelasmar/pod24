...
it('seeds 7 FAQs, 3 testimonials, and 4 use cases', function () {
    $this->seed(\Database\Seeders\Pod24ContentSeeder::class);

    expect(FaqItem::count())->toBe(7);
    ...
    expect(FaqItem::where('question->en', 'Is Pod24 a video or audio podcast studio?')->exists())->toBeTrue();
    expect(UseCase::where('title->en', 'Interview shows')->exists())->toBeTrue();
});
